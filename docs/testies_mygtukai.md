...
+ Kitas būdas surinkti kirčiuotas raides yra naudoti kirčių uždėtinius ženklelius (jie yra mygtukų __X__, __H__ ir __Q__ trečiajame lygyje), dedant juos ant prieš tai įvestos raidės. Tokius uždėtinius ženklelius turi daugumas šiuolaikinių spaudmenų (ù, ã, ū̃, ó, ė̃, į̃, ì, i̇̀ (čia: i + uždėtinis taškas + uždėtinis kairinis kirtis). Uždėtinių ženklelių atvaizdavimo kokybė įvairuoja nuo skirtingų spaudmenų ir skirtingos skaitmeninės įrangos: uždėtinis ženklelis gali būti paslinktas į šoną, ar uždėtas per aukštai, arba per žemai ant jau esančio kito uždėtinio ir priklausančio tai raidei ženklelio (taško, brūkšnio). Panašu, kad dar yra skirtingas supratimas tarp skirtingų kalbų vartotojų, kaip turėtų būti uždedami tie ženkleliai.
...
- ← → ↺ (PRIEŠLAIKRODINĖ ATVIRA RATULINĖ RODYKLĖ)
- ą → α (GRAIKŲ MAŽOJI RAIDĖ ALPHA)
- b → β (GRAIKŲ MAŽOJI RAIDĖ BETA)
- → → ↻ (PALAIKRODINĖ ATVIRA RATULINĖ RODYKLĖ)
...
- ↓ → ꜜ (KEITIKLINĖ RAIDĖ PAKELTA RODYKLĖ ŽEMYN)
- ↑ → ꜛ (KEITIKLINĖ RAIDĖ PAKELTA RODYKLĖ VIRŠUN)
...
- — → ↔ (KAIRINIS DEŠININIS RODYKLĖ)
- l → ▽ (BALTASIS ŽEMYNKRYPTIS TRIKAMPIS)
- ␣ → ˆ (STOGELIO KIRTIS)
- ‚ → ˌ (KEITIKLINĖ RAIDĖ STAČIAS BRŪKŠNELIS ŽEMAI)
- ‘ → ˈ (KEITIKLINĖ RAIDĖ STAČIAS BRŪKŠNELIS)
...
- | → ⟊ (STAČIA LAZDA SU GULSČIU PERBRAUKIMU)
- v → ⊽ (NOR)
...
- : → ̏ (UŽDĖTINIS DVIGUBAS KAIRINIS KIRTIS)
...
- ↓ → ⭏ (TRUMPA PIETUOSNA LENKTA RODYKLĖ)
- ↑ → ⭎ (TRUMPA ŠIAURĖN LENKTA RODYKLĖ)
...
- : → ̋ (UŽDĖTINIS DVIGUBA DEŠININIS KIRTIS)
...
- — → ↮ (KAIRINIS DEŠININIS RODYKLĖ SU PERBRAUKIMU)
- [ → ⁅ (KAIRINIS KVADRATAS SKLIAUSTAS SU SPYGLIU)
- ← → ↚ (KAIRINISWARDS RODYKLĖ SU PERBRAUKIMU)
...
- ' → ′ (PIRMINIS)
- ” → ⁗ (KETURGUBAS PIRMINIS)
- ] → ⁆ (DEŠININIS KVADRATAS SKLIAUSTAS SU SPYGLIU)
- → → ↛ (DEŠININISWARDS RODYKLĖ SU PERBRAUKIMU)
...
- ␣ → ¨ (DUTAŠKIS)
- ! → ‼ (DVIGUBA SUŠUKIMO ŽYMĖ, DVIGUBAS ŠAUKTUKAS)
- _ → ‗ (DVIGUBA ŽEMA LINIJA)
- ? → ⁇ (DVIGUBA KLAUSIMO ŽYMĖ, DVIGUBAS KLAUSTUKAS)
- \ → ⋱ (DEŠINĖJE NULEISTAS SKERSAS TRITAŠKIS)
- ↓ → ⇓ (ŽEMYN RODANTI DVIGUBA RODYKLĖ)
...
- ( → ⸨ (KAIRINIS DVIGUBAS SKLIAUSTELIS)
- ← → ⇐ (KAIRĖN PAKREIPTA DVIGUBA RODYKLĖ)
- ⟨ → ⟪ (MATEMATINIS KAIRINIS DVIGUBAS KAMPINIS SKLIAUSTAS)
- [ → ⟦ (MATEMATINIS KAIRINIS BALTASIS KVADRATINIS SKLIAUSTAS)
- ⟩ → ⟫ (MATEMATINIS DEŠININIS DVIGUBAS KAMPINIS SKLIAUSTAS)
- ] → ⟧ (MATEMATINIS DEŠININIS BALTASIS KVADRATINIS SKLIAUSTAS)
- – → ⋯ (VIDURINIS GULSČIAS TRITAŠKIS)
- > → ≫ (DAUG DAUGIAU NEI)
- < → ≪ (DAUG MAŽIAU NEI)
...
- ¬ → ⌐ (APSUKTAS NEIGIMO ŽENKLAS)
- ¶ → ⁋ (APSUKTAS PASTRAIPOS ŽENKLAS)
- ) → ⸩ (DEŠININIS DVIGUBAS SKLIAUSTELIS)
- → → ⇒ (DEŠINĖN PAKREIPTA DVIGUBA RODYKLĖ)
- * → ⁑ (DVI ŽVAIGŽDUTĖS RIKIUOTOS STAČIAI)
- / → ⋰ (DEŠININĖJE PAKELTAS SKERSAS TRITAŠKIS)
- ↑ → ⇑ (DVIGUBA RODYKLĖ VIRŠUN)
...
- ↓ → ⭝ (PIETUOSNA LENKTA RODYKLĖ SU GULSČIA UODEGA)
...
- = → ̳ (UŽDĖTINĖ DVIGUBA ŽEMA LINIJA)
...
- j → ̡ (UŽDĖTINIS ŽENKLAS SUMINKŠTINIMO KABĖ APAČIOJE)
...
- _ → ͜  (UŽDĖTINIS DVIGUBA LANKELIS APAČIOJE)
- : → ̑ (UŽDĖTINIS APVERSTAS LANKELIS)
- ; → ̯ (UŽDĖTINIS APVERSTAS LANKELIS APAČIOJE)
- ~ → ̾ (UŽDĖTINIS ŽENKLAS STAČIA BANGELĖ)
